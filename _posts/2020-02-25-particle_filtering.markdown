...
2.修改weights的分布为帕累托分布（当前使用的是正态分布）
   ...
   2.我首先分析了高斯分布，先上图![高斯分布](https://s2.ax1x.com/2020/02/26/3NiijS.png)，请忽略图上的坐标，与本题无关。
...
4. 修改particle filtering过程，消除随机误差对定位结果的影响

   我仔细思考了如何消除随机误差，回想起我大学选修的深度学习中消除误差的方法（虽然一个是机器学习，一个是深度学习，但我认为消除误差的思想是可以借鉴的），在线性变换后面加上非线性变换
